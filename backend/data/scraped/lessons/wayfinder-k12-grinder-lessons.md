...
- last_calibration_change: 2026-04-26 — Run 27 launched TX-high state with top-50-by-enrollment approach; 8/8 verified, 0 skipped on the 8 largest TX comprehensive HS. **Top-of-state-by-enrollment yields essentially 100% verification rate** because the largest schools are always comprehensive 9-12s with rich district websites, US News pages, Niche pages, and Texas Tribune profiles. Recommend bumping batch_size to 10 for Run 28 while remaining in TX top-50 band (next 8 still comprehensive: Deer Park, Permian, Coppell, Hastings, Klein Cain, Odessa, Dobie, Atascocita).
...
- **NEW (Run 22):** **Evergreen SD (Clark County) single-domain cluster** — offsets 194-201 are 7 schools all on `*.evergreenps.org` subdomains plus 1 vocational consortium (Cascadia Tech). One round of `site:evergreenps.org "[School]" principal 2025` queries surfaced principal + AP courses + grad rate + Niche/USNews ranking in single-snippet form for all 7 (8th was Cascadia which uses its own domain). 0 fetch calls needed. ~6 min wall-clock for 8 schools — fastest run yet.
...
- **NEW (Run 27):** **First TX-high run — top-50-by-enrollment approach kicks off.** Offsets 0-7 by enrollment DESC = the 8 largest TX comprehensive HS (Allen 5317, Conroe 4915, North Shore 4569, Duncanville 4501, Woodlands 4361, Lewisville 4240, Skyline 3780, Elsik 4118). All 8 verified, 0 skipped. Pattern: when sorted by enrollment, top of TX queue is dense with 4000+-student suburban-to-urban comprehensive HS — none alt/special/CTE — yielding 100% verification rate. Stark contrast to WA-by-NCES-offset which intermixed alt/skills-center/Open-Doors clusters at every 8-row band.
- **NEW (Run 27):** **Multi-campus TX HS pattern — extremely common at 4000+ enrollment.** 5 of 8 schools this run use a 9th-Grade-Center + Senior-Campus split (Allen, Conroe, North Shore, Woodlands, Lewisville). Allen + Lewisville go further: Lewisville has Killough (9-10 N) + Harmon (9-10 S) + Main (11-12). The "principal" field for these is the main-campus head; record `principalNote` describing the multi-campus structure. NCES enrollment for the *main* campus row is correct (it doesn't double-count freshman center).
- **NEW (Run 27):** **Finalsite-hosted TX district sites parse cleanly via direct WebFetch when targeted at the principal's-message page** (not the staff directory). lhs.lisd.net/our-school/administration/principals-message returned principal name + signature image + email in clean HTML. The /administration parent page only listed assistant principals with no head-principal block; the /principals-message child page had the signed letter. Pattern: for any LISD/Finalsite TX site where /administration shows only AP list, jump directly to /principals-message.
- **NEW (Run 27):** **Skyline HS (Dallas ISD) is officially a magnet** — Career Development Center (CDC) with 26+ career clusters (architecture, aviation, fashion, IT, etc.). Tagged `magnet: true` + `magnetProgram: true` + `schoolType: "career_magnet"` per Run 22 metadata pattern. Even though NCES `magnet: false` field, the school is widely recognized as a citywide career magnet — when NCES disagrees with school's own description, prefer the school's self-description and add `magnetProgram: true` flag.
...
- **NEW (Run 22, replaces Run 21 suggestion):** Run 22 was the second 8/8 verified, 0-skipped batch on record (after Run 14). The Evergreen SD single-domain cluster made this run essentially free — all WebSearch, ~6 min total. **Strong recommendation: bump batch_size to 10 for Run 23** while the queue stays in the clean WA-high band (Federal Way → Fife → Granger likely all comprehensive HS in well-mapped districts). If skip rate stays at 0-1 of 10 across Run 23 + Run 24, consider holding at 10. Revert to 8 if the queue hits a Skills Center / alt-placeholder cluster.
- **NEW (Run 22):** The dedup pre-filter (Run 9's open proposal) is STILL unimplemented but Run 22 had 0 dups so the issue isn't biting yet. Will likely bite again in the WA-high G-N range when manual entries from prior weeks (Bellevue, Interlake, Newport, etc.) cluster. Codification recommendation: at batch-selection, run `existing_ncessch.has(candidate.ncessch)` → if true, advance offset silently; track `pre_dedup_skipped` separately from research-level skips.
