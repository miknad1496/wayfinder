...
- **Re-cloning into /tmp**: the cloned tree is owned by `nobody:nogroup` and not writable by `great-brave-faraday`. Permanent workaround: copy tree into `/sessions/.../mnt/outputs/wayfinder-esms` (writable), do edits + commit there, push from that copy.
