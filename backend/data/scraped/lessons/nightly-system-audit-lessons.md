...
- **NEW (2026-05-04)**: `verifyToken` token cache TTL semantics — is there an upper bound on cache size, or could a long-lived production process accumulate stale token entries forever? Worth investigating in the next scheduled deep-auth audit slot.
