...
/* === REVAMP V2: PLAN PRINT / EMAIL / COPY ACTIONS === */
// Plain-text version of a plan response. Used by Email + Copy actions.
function _scPlanToPlainText(data) {
  const p = (data && data.plan) || {};
  const out = [];
  out.push('YOUR SUMMER PLAN — Wayfinder K-8');
  out.push('=================================');
  out.push('');
  if (p.summary) out.push('SUMMARY: ' + p.summary);
  if (data && data.calibrationInsight) {
    out.push('');
    out.push('2026 CALIBRATION FOR YOUR FAMILY:');
    out.push(data.calibrationInsight);
  }
  if (p.anchorRecommendation) {
    out.push('');
    out.push('ANCHOR COMMITMENT — ' + (p.anchorRecommendation.type || ''));
    if (p.anchorRecommendation.rationale) out.push(p.anchorRecommendation.rationale);
    if (Array.isArray(p.anchorRecommendation.lookFor) && p.anchorRecommendation.lookFor.length) {
      out.push('Look for: ' + p.anchorRecommendation.lookFor.join(' · '));
    }
  }
  if (Array.isArray(p.specialtyRecommendations) && p.specialtyRecommendations.length) {
    out.push('');
    out.push('SPECIALTY EXPERIENCES:');
    for (const sp of p.specialtyRecommendations) {
      out.push('');
      out.push('• ' + (sp.category || '') + (sp.estimatedCost ? ' — ' + sp.estimatedCost : ''));
      if (sp.rationale) out.push('  ' + sp.rationale);
      if (Array.isArray(sp.lookFor) && sp.lookFor.length) {
        out.push('  Look for: ' + sp.lookFor.join(' · '));
      }
    }
  }
  if (p.wildcardSuggestion) {
    out.push('');
    out.push('WILDCARD IDEA: ' + (p.wildcardSuggestion.idea || ''));
    if (p.wildcardSuggestion.rationale) out.push(p.wildcardSuggestion.rationale);
  }
  if (p.scholarshipNote) {
    out.push('');
    out.push('SCHOLARSHIP GUIDANCE: ' + p.scholarshipNote);
  }
...
function renderSCPlan(data) {
  ...
  document.getElementById('scPlanResult').innerHTML = `
    ...
    <div class="sc-plan-action-bar" style="display:flex;gap:8px;margin-top:14px;flex-wrap:wrap;border-top:1px solid #e5e7eb;padding-top:14px;">
      <button class="sc-plan-print-btn" style="font-size:13px;padding:8px 14px;border:1px solid #cfdcef;background:#fff;color:#1e40af;border-radius:6px;cursor:pointer;font-weight:600;">🖨️ Print / Save as PDF</button>
      <button class="sc-plan-email-btn" style="font-size:13px;padding:8px 14px;border:1px solid #cfdcef;background:#fff;color:#1e40af;border-radius:6px;cursor:pointer;font-weight:600;">✉️ Email this plan</button>
      <button class="sc-plan-copy-btn" style="font-size:13px;padding:8px 14px;border:1px solid #cfdcef;background:#fff;color:#1e40af;border-radius:6px;cursor:pointer;font-weight:600;">📋 Copy as text</button>
    </div>
  `;
  // Stash for action handlers (Email / Copy read from this).
  window._scLastPlanData = data;
}
...
/* === REVAMP V2: PLAN PRINT / EMAIL / COPY ACTIONS === */
// Print / Email / Copy handlers for the K-8 Plan output.
document.addEventListener('click', function(e) {
  const target = e.target;
  if (!target || !target.classList) return;
  if (target.classList.contains('sc-plan-print-btn')) {
    try { window.print(); } catch (err) { console.warn('print failed', err); }
    return;
  }
  if (target.classList.contains('sc-plan-email-btn')) {
    const data = window._scLastPlanData;
    if (!data) { alert('No plan data to email yet.'); return; }
    const body = _scPlanToPlainText(data);
    const subject = 'My Wayfinder Summer Plan';
    const MAX = 1900;
    let safeBody = body;
    if (safeBody.length > MAX) safeBody = safeBody.slice(0, MAX) + '\n\n[truncated — see Wayfinder for full plan]';
    const url = 'mailto:?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(safeBody);
    window.location.href = url;
    return;
  }
  if (target.classList.contains('sc-plan-copy-btn')) {
    const data = window._scLastPlanData;
    if (!data) { alert('No plan data to copy yet.'); return; }
    const text = _scPlanToPlainText(data);
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(() => {
        ...
        alert('Copy failed — your browser blocked clipboard access. Try the Email button instead.');
      });
    } else {
      try {
...
async function loadApExamsAndTypes() {
  try {
    const [er, tr] = await Promise.all([
      fetch(API_BASE + '/ap-coach/exams'),
      fetch(API_BASE + '/ap-coach/frq-types'),
    ]);
