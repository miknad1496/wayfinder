...
## 2026-07-26 Nightly audit
**NOT CLEAN (carried)** -- 0 NEW code/data defects, but **major scheduler-narrative CORRECTION** + sharpened data escalation #1. Focus: cost, runtime[FULL BOOT], data[git-identity + fresh authoritative DNS re-derivation], scheduler-health[direct scheduled-tasks read], cross-cutting/security.

### HEADLINE CORRECTION -- data-refresh and pii-audit are NOT "missed/dead"; commit-cadence != run-cadence
This run had the `scheduled-tasks` MCP available (prior git-clone-isolated nightly runs did NOT). Reading `list_scheduled_tasks` directly overturns ~15 prior entries that inferred task health from git commit cadence:
- **wayfinder-data-refresh: `enabled:true`, `lastRunAt:2026-07-19T16:03Z`, `nextRunAt:2026-07-26T16:03Z` (today 9:03am).** It is NOT disabled and NOT dead -- it fires every Sunday. The "dead since 5/17 / 10 missed Sundays" framing carried since 5/25 is a MISDIAGNOSIS. Reality: the task fires weekly but its runs fail *before completing any grinder-write*. `.refresh-state.json` still reads `lastRunISO:2026-05-10T16:30Z, totalRuns:2` -- so no run since 5/10 has completed even the state-file write. The grinder-write endpoint + auth are intact in code (admin.js:982, INTERNAL_TASK_TOKEN). The task's own conservative guards ("if unsure -> STOP, better to skip a week", SKILL.md:151) mean failures exit silently with no notification -- a direct **"Failure visibility rule" violation** (CLAUDE.md scheduled-task rule 4). Root cause (WebFetch/WebSearch failing in the autonomous context, token mismatch on Render, or per-task tool-approval pause) requires the 7/19 run logs -- Dan's Cowork dashboard.
- **wayfinder-pii-audit: `enabled:true`, `lastRunAt:2026-07-25T11:04Z` (fired yesterday).** The "2 consecutive missed Saturdays (7/18,7/25)" conclusion is also a commit!=run artifact -- 7/25 DID run; a no-op run with zero PII to patch legitimately produces no commit (consistent with its own 7/13 lessons: "traffic is genuinely zero").
- **nightly-audit (me):** 7/24 genuinely has no commit (a clean/carried run always commits markdown, so a missing commit here IS a missing successful run, unlike the two tasks above). So the 7/25 entry's "healthy 5-night streak 7/21-7/25" was inaccurate -- 7/20 AND 7/24 were missed; firing is intermittent (~5 of last 7 nights), recovering but not yet stably nightly.
- **wayfinder-morning-pulse: `enabled:true`, fired 7/25** -- daily digest is alive, so the long-proposed dead-man's-switch (assert HEAD carries a data-refresh success within N days; flag absence in the email) has a live host to run in.
- Penserra tasks (tax/financial/estate/zakat-data-hardening, daily-platform-audit, *-brain, ii-scraper-watchdog, travelers-savings-autofill) observed present/enabled -- NOT touched.

### Carried escalation #1 -- NXDOMAIN _verified _source hosts (from 7/14), UNCHANGED but SHARPENED
Data is git-identical since 5/10 (empty `git diff` on all scraped JSONs), so the finding cannot have drifted. Re-derived fresh from source tonight (per the 7/19 "re-derive carried claims periodically" pattern) with an apex-vs-subdomain split + authoritative recheck vs 8.8.8.8 with retries. Result is cleaner than the carried "37 NXDOMAIN":
- **28 apex-DEAD** (programs 22 / internships 5 / volunteer 1) -- the registrable domain itself is NXDOMAIN on authoritative recheck. This is the hard fabrication core. Includes obvious typosquats: `nytimes.edu` (NYT is .com), `wheretheresbedragons.com` (real org is where**therebe**dragons.com), `junachievement.org` (real is juniorachievement.org). Most also carry the escalation-#2 templated `/learn/hs*` path -> the two escalations are largely the SAME fabricated international-HS batch seen from two angles.
- **13 subdomain-only-DEAD** -- apex is a LIVE real institution (upenn.edu, ucf.edu, fsu.edu, utexas.edu, rockefeller.edu, korea.ac.kr, asee.org/Navy SEAP, jazz.org/Lincoln Center, wonderopolis.org, bikeleague.org, esl.gg, boyshopegirlshope.org) but the cited program subdomain no longer resolves. These are REAL programs (RockEDU SSSRP, UPenn M&T, Jazz@Lincoln Summer Academy, NSWC Dahlgren SEAP x2, FSU YSP, UCF Upward Bound, Korea U Summer). **A naive bulk `_verified:false` NXDOMAIN sweep would WRONGLY nuke these** -- they need URL updates, not un-verification.
- **2 transient false-positives excluded** (empatico.org, jetblueforgood.com) -- failed the parallel first pass, resolved LIVE on authoritative retry. (Prior "37" likely folded some of these + subdomain cases together without the retry filter.)

Remediation-ready split (Dan's write-time decision): apex-dead 28 -> bulk `_verified:false` candidates + write-time DNS gate; subdomain-dead 13 -> individual re-verify/URL-update, do NOT un-verify. Full entry-level list (name + dead _source) captured in this run; the apex-dead programs cluster is: cmorkids, digiteknology, decacanada, lyceehenri4, hablayucatan, usgeolympiad, haguelaw, operationgroundswell, wheretheresbedragons, ieoglobal, igeo-olympiad, iesoinfo, iceland-school, borneorainforest, israeltech-foundation, almacuoco, cathayaviationacademy, lighthouse-foundation, crans-montana-ski-academy, klosters-davos-ski, stantonarlberg-academy, grindelwald-mountaineering.

### Carried escalation #2 -- templated-path program entries (from 6/18), UNCHANGED
Re-derived from source: 120 verified programs share the exact `/learn/hs*` templated `_source` path; the broader `international/hs batch` `_addedBy` tag covers 320; carried "~221" sits in that band. Git-identical since 5/10. Substantially overlaps escalation #1's apex-dead programs -- same batch.

### CLEAN -- everything else (no new defects)
- **Cost**: SLM keep-alive ping does NOT touch `lastWarmAt` (slm.js:871-872 comment-guard; keepalive self-terminates via MAX_IDLE at 842-845). 4 backend setIntervals -- 3 with clearInterval (user-backup 262/279, scraper-scheduler 258/268, slm 840/844) + 1 intentional daemon (scheduler.js:184). Anon daily cap `ANON_DAILY_LIMIT=5` disk-persisted AND enforced (chat.js:329 -> 429). Rate tiers: `effectiveMax` 30 auth / 5 anon (chat.js:346). Opus only on gated paths (essay-reviewer, ap-coach, head-consultant supplement, engine mode); standard chat = sonnet, classifiers/advisor = haiku.
- **Runtime**: FULL native boot clean (`npm i` 143 pkgs + `node server.js`). Data-health: internships 1606 (981v), scholarships 1043 (80v), programs 1416 (672v) -- all "clean". ApCoach (220 units/37 exams) + intl-brain(korea) init OK. Graceful SIGTERM + final backup. Disk 63%, no ENOSPC.
